  ...
  container_properties = <<CONTAINER_PROPERTIES
{
    ...
    "image": "${aws_ecr_repository.ecr_repos.repository_url}:${data.external.git_describe.result["academy_capstone_version"]}",
    "memory": 6000,
    "vcpus": 4,
...
data "aws_iam_policy_document" "secret_manager" {
  statement {
    ...
    resources = [
      "arn:aws:secretsmanager:eu-west-1:${local.account_id}:secret:${local.secret_name}*"
    ]
    effect = "Allow"
  }
}
